...
class DomainZone(Base):
    """
    DomainZone model representing a Cloudflare domain zone.
    """
...
class DNSRecord(Base):
    """
    DNSRecord model for storing Cloudflare DNS records.
    """
    ...
    name = Column(String(255), nullable=False)
    type = Column(String(10), nullable=False)  # A, CNAME, TXT, etc.
...
class DomainServiceMapping(Base):
    ...
    id = Column(Integer, primary_key=True, index=True)
    zone_id = Column(Integer, ForeignKey("domain_zones.id", ondelete="CASCADE"))
    service_name = Column(String(50), nullable=False)  # api, web, mqtt, nextcloud, etc.
