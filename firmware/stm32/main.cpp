/**
 * SwissAirDry Platform - STM32 Firmware
 * 
 * This firmware is designed for STM32-based SwissAirDry devices.
...
#define DEVICE_TYPE "stm32"
#define DEFAULT_DEVICE_NAME "SwissAirDry-STM32"
...
void setup() {
  ...
  Serial.begin(115200);
  Serial.println("\n\nSwissAirDry STM32 Firmware Starting...");
  ...
  if (config.deviceName.length() == 0) {
    ...
    config.deviceName += "-" + String(getSTM32SerialNumber(), HEX);
  }
  
  // Set default device ID if not configured
  if (config.deviceId.length() == 0) {
    config.deviceId = "stm32-" + String(getSTM32SerialNumber(), HEX);
  }
...
void loop() {
  ...
  // Yield to allow STM32 to handle background tasks
  yield();
}
...
void setupWiFi() {
  ...
  String apName = "SwissAirDry-" + String(getSTM32SerialNumber(), HEX);
