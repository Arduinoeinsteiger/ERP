/**
 * SwissAirDry Platform - ESP32 Firmware
 * 
 * This firmware is designed for ESP32-based SwissAirDry devices.
...
#define DEFAULT_DEVICE_NAME "SwissAirDry-ESP32"
...
void setup() {
  ...
  Serial.begin(115200);
  Serial.println("\n\nSwissAirDry ESP32 Firmware Starting...");
  ...
  // Setup display (128px OLED for ESP32)
  display.init(DISPLAY_128PX);
...
void loop() {
  ...
  // Short delay to prevent watchdog timer reset
  delay(10);
}
...
void setupWiFi() {
  ...
  String apName = "SwissAirDry-" + String((uint32_t)(ESP.getEfuseMac() >> 32), HEX);
...
void setupSensors() {
  ...
  // Set analog pin resolution for pressure and current sensors
  analogReadResolution(12); // 12-bit resolution for ESP32 (0-4095)
  ...
  if (isnan(h) || isnan(t)) {
    ...
    display.showError("DHT sensor error");
  } else {
    Serial.println("Temperature: " + String(t) + "°C");
    Serial.println("Humidity: " + String(h) + "%");
  }
  
  // Test reading pressure sensor
  int pressureRaw = analogRead(PRESSURE_SENSOR_PIN);
  currentPressure = map(pressureRaw, 0, 4095, 900, 1100) / 10.0; // Map to hPa (900-1100)
  Serial.println("Pressure: " + String(currentPressure) + " hPa");
  
  // Test reading current sensor
  int currentRaw = analogRead(CURRENT_SENSOR_PIN);
  currentConsumption = map(currentRaw, 0, 4095, 0, 1000) / 100.0; // Map to Watts (0-10W)
  Serial.println("Power consumption: " + String(currentConsumption) + " W");
}
...
void setupControls() {
  ...
  ledcAttachPin(FAN_CONTROL_PIN, pwmChannel);
...
void publishTelemetry() {
  ...
  if (isnan(humidity) || isnan(temperature)) {
    Serial.println("Failed to read from DHT sensor!");
    // Don't return here as we might still have valid pressure and current readings
    humidity = 0;
    temperature = 0;
  }
  ...
  display.showAdditionalData(currentPressure, currentConsumption);
  ...
  doc["fan_speed"] = currentFanSpeed;
  doc["power_consumption"] = currentConsumption;
...
void handleConfigMessage(String message) {
  ...
  if (doc.containsKey("display_type")) {
    ...
    if (displayType == "128px" || displayType == "none") {
      // Only 128px and none are supported on ESP32
